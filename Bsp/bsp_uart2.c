...
static volatile uint8_t uart2_rx_recover_pending;

BSP_Uart2Diagnostics_t bsp_uart2_diagnostics;
...
void BSP_Uart2_Init(void)
{
  ...
  uart2_rx_head = 0U; uart2_rx_tail = 0U; uart2_rx_recover_pending = 0U;
  bsp_uart2_diagnostics = (BSP_Uart2Diagnostics_t){0};
...
{
  if (uart2_rx_recover_pending != 0U)
  {
    uart2_rx_recover_pending = 0U;
    (void)HAL_UART_AbortReceive(&huart2);
    uart2_rx_head = 0U;
    uart2_rx_tail = 0U;
    if (HAL_UART_Receive_IT(&huart2, &uart2_rx_byte, 1U) != HAL_OK)
    {
      uart2_rx_recover_pending = 1U;
    }
    else
    {
      bsp_uart2_diagnostics.recover_count++;
    }
  }
...
uint8_t BSP_Uart2_Send(const uint8_t *data, uint16_t length)
{
  ...
  if (data == NULL || length == 0U || length > BSP_UART2_TX_MAX_LENGTH)
  {
    bsp_uart2_diagnostics.tx_drop_count++;
    return 0U;
  }
...
void BSP_Uart2_OnRxCplt(UART_HandleTypeDef *huart)
{
  ...
  {
    bsp_uart2_diagnostics.rx_drop_count++;
  }
  (void)HAL_UART_Receive_IT(&huart2, &uart2_rx_byte, 1U);
}
...
void BSP_Uart2_OnError(UART_HandleTypeDef *huart)
{
  ...
  {
    bsp_uart2_diagnostics.error_count++;
    bsp_uart2_diagnostics.last_error_code = HAL_UART_GetError(huart);
    uart2_rx_recover_pending = 1U;
  }
}
