...
`shoot_enabled` 是比赛射击许可：准备模式或正式比赛中，机器人存活且未判负时为 1。它与 `power_on` 完全独立；`power_on` 只反映底盘供电输出是否打开。
...
`result`：0=成功，1=不允许，2=失败。相同命令和 transaction_id 重复到达时不重复执行，只重发原回执。设备上电默认进入准备模式，L431PM 自行设置 `HP=200、alive=1`；准备模式已经启用受击扣血、死亡、5 秒复活和射击许可，便于没有服务器时联调。准备模式中的 `SET_HP` 合法范围是 `0~200`。服务器发送比赛开始后，L431PM 自行切换为正式比赛，设置 `HP=300、alive=1`，清除本场黄牌和判负状态；正式比赛中的 `SET_HP` 合法范围是 `0~300`，HP 归零后等待 5 秒自动复活至 300。每次有效受击扣 20。黄牌处罚由服务器每次发送一条 `C4` 命令：本场第 1、2 次各扣 50 HP，第 3 次直接判负并不再自动复活。黄牌计数、HP 扣除、复活和判负均由 L431PM 执行，ESP32 只转发命令。
